...
bool	CInput::GetReadingWindowOrientation()
{
    ...
    if( !bHorizontalReading && ( m_dwId[0] & 0x0000FFFF ) == LANG_CHT )
    {
        ...
        strcpy_s( szRegPath, _countof(szRegPath), "software\\microsoft\\windows\\currentversion\\" );
        strcat_s( szRegPath, _countof(szRegPath), ( dwVer >= MAKEIMEVERSION( 5, 1 ) ) ? "MSTCIPH" : "TINTLGNT" );
		...
		if(lpIC == NULL) {
			...
			switch( m_dwId[0] )
			{
				...
				case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
					{
						bool isVistaOrGreater = IsWindowsVistaOrGreater();

						int nTcharSize = isVistaOrGreater ? sizeof(wchar_t) : sizeof(char);
						p = *(LPBYTE *)((LPBYTE)g_imm.LockIMCC( lpIC->hPrivate ) + 1*4 + 1*4 + 6*4);
						if( !p ) break;
						tempLen = *(DWORD *)(p + 1*4 + (16*2+2*4) + 5*4 + 16 * nTcharSize);
						dwErr = *(DWORD *)(p + 1*4 + (16*2+2*4) + 5*4 + 16 * nTcharSize + 1*4);
						temp  = (wchar_t *) (p + 1*4 + (16*2+2*4) + 5*4);
						bUnicodeIme = isVistaOrGreater;
					}
					break;
